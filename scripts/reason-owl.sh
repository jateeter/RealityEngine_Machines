...
CORPUS_SHA="$(cd "$REPO_ROOT" && git rev-parse --short HEAD 2>/dev/null || echo unknown)"
...
"$ROBOT" annotate --input "$WORKDIR/merged.owl" \
  ...
  --annotation "http://purl.org/dc/terms/created" "$RELEASE_DATE" \
  --annotation "http://purl.org/dc/terms/source" "RealityEngine_Machines@$CORPUS_SHA" \
  ...
  --output "$ANNOTATED"
echo "reason-owl: annotated — version $RELEASE_DATE, corpus $CORPUS_SHA, ${INFO_COUNT} INFO / ${WARN_COUNT} WARN pending triage"
